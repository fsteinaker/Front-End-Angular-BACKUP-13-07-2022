...
explaboralList!: explaboral[];
  ...
  constructor(public explaboralService : PortfolioService, public router: Router) { }
  ...
  ngOnInit(): void {
    ...
    agregarExperiencia(){
    this.router.navigateByUrl('/', {skipLocationChange: true}).then(() => this.router.navigate(['/nueva-exp']));
    }
    ...
    editarExperiencia(id:number) {
      this.router.navigateByUrl('/', {skipLocationChange: true}).then(() => this.router.navigate(['/editar-exp',id]));
    }
