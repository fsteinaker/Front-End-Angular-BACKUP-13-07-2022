...
export class EditarExpComponent implements OnInit {
  ...
  constructor(private explaboralService: PortfolioService, private router: Router, private route:ActivatedRoute) { }

  ngOnInit() {
    this.id = this.route.snapshot.params['id'];
    this.explaboralService.detalleExplaboral(this.id).subscribe(
      (dato: explaboral) => {
  ...
  onSubmit(){
    this.explaboralService.updateExplaboral(this.id,this.explaboral).subscribe(
      dato => {
      this.irHome();
    });
